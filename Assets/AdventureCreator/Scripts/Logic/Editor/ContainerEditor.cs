	...
	public override void OnInspectorGUI()
	{
		...
		if (_target.items.Count > 0)
		{
			...
			for (int i=0; i<_target.items.Count; i++)
			{
				...
				{
					EditorGUILayout.LabelField (inventoryManager.GetLabel (_target.items[i].linkedID), EditorStyles.boldLabel, GUILayout.Width (180f));
					EditorGUILayout.LabelField ("Count:", GUILayout.Width (50f));
					_target.items[i].count = EditorGUILayout.IntField (_target.items[i].count, GUILayout.Width (94f));
				}
				else
				{

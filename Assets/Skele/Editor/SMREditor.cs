...
public class SMREditor : Editor
{
	...
	#region "Pose Manager"
	// "Pose Manager" 
    private static JointList ms_MultiSelectJoints; //the multi-selection joints
    ...
    void OnSceneGUI()
    {
        ...
        Tools.current = UnityEditor.Tool.None; // clear the default position/rotation/scale handles
        ...
        switch (e.type)
        {
            ...
            case EventType.Layout:
                {
                    if( ms_SelectedBone != ms_SelectedBone_Req )
                    {
                        _OnChangeSelectedBone(ms_SelectedBone, ms_SelectedBone_Req);
                    }
        ...
        else if ( ms_eOpMode == EOPMode.IK_Pinned ) //Route-C
        {
            ...
            {
                Quaternion rot = _GetQuaternionByPivotRotation(theRoot);
            ...
            // C.2
            {
                ...
                if (ms_SelectedBone != null && ms_SelectedBone != theRoot)
                { 
                    ...
                    if(bValidTarget)
                    {
                        Handles.BeginGUI();
                        {
                            Vector2 pt = HandleUtility.WorldToGUIPoint(ms_SelectedBone.position) + new Vector2(10, 10);
                            EUtil.PushGUIColor(Color.green);
                            if (GUI.Button(new Rect(pt.x, pt.y, 40, 40), "V"))
                            {
                                _AddIKPin(ms_SelectedBone);
        ...
        else if (ms_eOpMode == EOPMode.PoseManage ) //Route-E
        {
            ...
            if( e.type == EventType.MouseUp && !ms_bRightBtnIsDown && e.button == 0 && !ms_bHasModalWindow)
            {
                ...
                if( null != joint )
                {
                    if (e.shift)
                    {
                        if( ms_MultiSelectJoints.Contains(joint) )
                        {
                            _RemoveJointFromSelection(joint);
                        }
                        else
                        {
                            _AddJointIntoSelection(joint);
                        }
                    }
                    else
                    {
    ...
    public override void OnInspectorGUI()
    {
        ...
        if( ms_bEditing )
        {
            ...
            if (EUtil.Button(bHasTopmost ? "Resume SceneCam Speed" : "Slower SceneCam Speed", c))
            {
                if (bHasTopmost)
                {
    ...
    private static void _OnEndEdit()
    {
        ...
        ms_SelectedBone = ms_SelectedBone_Req = null;
    ...
    //    if (bReg)
    //        undoCallback.SetValue(null, (EditorApplication.CallbackFunction)_OnUndoRedo);
    //    else
    //        undoCallback.SetValue(null, (EditorApplication.CallbackFunction)null);
    //}
...
");
    ...
    private static void _DrawSceneGUI_Common()
    {
        GUILayout.BeginHorizontal();
        {
            ...
            if (EUtil.Button("C", "Focus the Camera to Selected Joint or Reset Camera"))
            {
                ...
                if( ms_SelectedBone != null )
                {
                    EUtil.SceneViewLookAt(ms_SelectedBone.position);
                }
                else
                {
                    EUtil.AlignViewToObj(ms_CurSelectedGO);
            ...
            msg = ms_LockSelection ? "Locked" : "Free";
    ...
    private static void _DrawSceneGUI_Normal()
    {
        ...
        GUILayout.BeginArea(rect);
        {
            ...
            if (ms_SelectedBone != null)
            {
                EditorGUILayout.LabelField(string.Format("SelectedBone: {0}", ms_SelectedBone.name));
                #if IMDEBUGGING
                Vector3 newLR = EditorGUILayout.Vector3Field("LocalRotate:", ms_SelectedBone.localEulerAngles);
                if( newLR != ms_SelectedBone.localEulerAngles )
                {
                    Undo.RecordObject(ms_SelectedBone, "DbgRotate");
                    newLR.x = Misc.NormalizeAngle(newLR.x);
                    newLR.y = Misc.NormalizeAngle(newLR.y);
                    newLR.z = Misc.NormalizeAngle(newLR.z);
                    ms_SelectedBone.localEulerAngles = newLR;
                }
                #endif
            }
        }
    ...
    private static void _DrawSceneGUI_IK()
    {
        ...
        GUILayout.BeginArea(mainUIRect);
        {
            ...
            {
                EditorGUILayout.LabelField(String.Format("Selected: {0}", ms_SelectedBone.name));
                GUILayout.BeginHorizontal();
                {
        ...
        if( ms_SelectedBone != null && ms_IKSolver.Count == 2)
        {
            ...
            GUILayout.BeginArea(cRect);
            {
                GUILayout.BeginHorizontal();
                {
                    bool bHasConstraint = ms_DictTr2Constraint.ContainsKey(ms_SelectedBone);
                    ...
                    if ( EUtil.Button("C", "Control_IK_Constraints", c))
                    {
                        ...
                        if( bHasConstraint )
                        {
                            ms_DictTr2Constraint.Remove(ms_SelectedBone);
                ...
                case KeyCode.Escape:
                    {
                        ...
                        {
                            ms_SelectedBone_Req = null; //clear the selected bone
                            return;
                        }
                    }
                    break;
	        ...
	        case EOPMode.IK:
	            {
                    ...
                    if (!_ChangeBone4IKLnk(ms_SelectedBone))
                        ms_SelectedBone = null;
	...
	private static void _OnChangeSelectedBone(Transform prevJoint, Transform newJoint)
	{
        ...
        switch( ms_CurPivotRotation )
        {
            case PivotRotation.Parent:
                {
                    if (ms_SelectedBone != null && ms_SelectedBone.parent != null)
                        ms_RotTmp = ms_SelectedBone.parent.rotation;
                }
                break;
        }
    }
    ...
    private static void _DrawMarkerForIKTarget()
    {
        ...
        {
            if (extTr.rotation != ms_SelectedBone.rotation)
            {
                extTr.rotation = ms_SelectedBone.rotation;
                HandleUtility.Repaint();
            }
        }
        ...
        {
            Dbg.LogErr("SMREditor._DrawMarkerForIKTarget: force snap to Move, unexpected HandleTool: {0}", ms_CurHandleTool);
            ms_CurHandleTool_Req = Tool.Move;
            _SceneRepaintAll();
        }
    ...
    private static void _CalcMarkersForVertices()
    {
        ...
        int selectedBoneIdx = Array.FindIndex(bones, x => { return x == ms_SelectedBone; });
        ...
        for (int vidx = 0; vidx < verts.Length; ++vidx)
        {
            ...
            if (bw.weight0 > 0)
            {
                ...
                fUnderSelectedBone += (bw.boneIndex0 == selectedBoneIdx) ? bw.weight0 : 0;
            }
            if (bw.weight1 > 0)
            {
                ...
                fUnderSelectedBone += (bw.boneIndex1 == selectedBoneIdx) ? bw.weight1 : 0;
            }
            if (bw.weight2 > 0)
            {
                ...
                fUnderSelectedBone += (bw.boneIndex2 == selectedBoneIdx) ? bw.weight2 : 0;
            }
            if (bw.weight3 > 0)
            {
                ...
                fUnderSelectedBone += (bw.boneIndex3 == selectedBoneIdx) ? bw.weight3 : 0;
            }

            Color c = new Color(fUnderSelectedBone, 1 - fUnderSelectedBone, 0);
    ...
    {
        return oneBone == ms_SelectedBone ? Color.blue : Color.yellow;
    }

    private static Color _ColorFunc_JointIsInMultiSelectJoint(Transform oneBone)
    {
        return ms_MultiSelectJoints.Contains(oneBone) ? Color.green : Color.yellow;
    }
            ...
            case Tool.Move:
                {
                    ...
                    {
                        Undo.RecordObject(tr, "Move Bone");
                        tr.position = newPos;
            ...
            case Tool.Rotate:
                {
                    if( ms_CurPivotRotation == PivotRotation.Local )
                    {
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            Undo.RecordObject(tr, "Rotate Bone");
                            tr.rotation = newRot;
                            EditorUtility.SetDirty(tr);
                            //ms_bNeedRecalcVerts = true;
                        }
            ...
            case Tool.Scale:
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObject(tr, "Scale Bone");
                ...
                break;
            default:
                Dbg.LogErr("SMREditor._ManipulateBones: unexpected Tool enum: {0}", ms_CurHandleTool);
                break;
        }
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        switch( ms_BindPoseFixerMode)
                        {
                            case EBindPoseFixerMode.NORMAL:
                                {
                                    Undo.RecordObject(tr, "Move BindPose");
                                    tr.position = newPos;
                                }
                                break;
                            case EBindPoseFixerMode.SOLE_JOINT:
                                {
                                    ...
                                    tr.position = newPos;
                                    _RevertChildrenJointWorldTransform(tr, ms_BindPoseFixerSkeleton.ToArray()); 
                                }
                                break;
                        }
                    ...
                    if (ms_CurPivotRotation == PivotRotation.Local)
                    {
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            switch (ms_BindPoseFixerMode)
                            {
                                case EBindPoseFixerMode.NORMAL:
                                    {
                                        Undo.RecordObject(tr, "Rotate BindPose");
                                        tr.rotation = newRot;
                                    }
                                    break;
                                case EBindPoseFixerMode.SOLE_JOINT:
    ...
    /// </summary>
    private void _SetSelectedJointByEvent(Event e, Transform[] bones)
    {
        ...
        if (e.type == EventType.MouseUp && !ms_bRightBtnIsDown && e.button == 0 && LMBTotalDownTime <= CLICK_THRES)
        {
            ...
            if (ms_SelectedBone != newJoint && newJoint != null)
            {
                ms_bNeedRecalcVerts = true;
                ms_SelectedBone_Req = newJoint;
            }
            _SceneRepaintAll();
        }
    }
    ...
    private static bool _TryMakeIKLink(ref ISolver isolver, Transform endJoint, int lnkLength)
    {
        ...
        {
            if( joint == null || !bones.Contains(joint) )
            {
                return false;
            }
    ...
    /// </summary>
    private static Quaternion _GetQuaternionByPivotRotation(Transform tr)
    {
        ...
        switch ( ms_CurPivotRotation )
        {
            ...
            case PivotRotation.Parent:
                ...
                break;
            default:
                Dbg.LogErr("SMREditor._GetQuaternionByPivotRotation: unexpected pivot rotation setting: {0}", ms_CurPivotRotation);
                break;
        }
    ...
    private void _SelectJointAndAllDescendants(Transform joint)
    {
        ...
        _Recur_SelectJointAndAllDescendants(joint);
    }
    private void _Recur_SelectJointAndAllDescendants(Transform curJoint)
    {
        ms_MultiSelectJoints.Add(curJoint);

        for( int idx = 0; idx < curJoint.childCount; ++idx )
        {
            Transform j = curJoint.GetChild(idx);
            _Recur_SelectJointAndAllDescendants(j);
        }
    }

    /// <summary>
    /// add the specified joint into MultiSelectionJoint
    /// </summary>
    private void _AddJointIntoSelection(Transform joint)
    {
        Dbg.Assert(!ms_MultiSelectJoints.Contains(joint), "SMREditor._AddJointIntoSelection: the joint already in selection: {0}", joint.name);

        ms_MultiSelectJoints.Add(joint);
    }

    /// <summary>
    /// remove the specified joint from MultiSelectionJoint
    /// </summary>
    private void _RemoveJointFromSelection(Transform joint)
    {
        Dbg.Assert(ms_MultiSelectJoints.Contains(joint), "SMREditor._RemoveJointFromSelection: the joint not in selection: {0}", joint.name);

        ms_MultiSelectJoints.Remove(joint);
    }
    ...
    private static PoseDataDict _GenPoseDataDictFromMultiSelect()
    {
        ...
        for(var ie = ms_MultiSelectJoints.GetEnumerator(); ie.MoveNext(); )
        {
    ...
    private static void _CreateNewPose(string poseName)
    {
        ...
        ms_PoseSet.AddPose(poseName, ms_MultiSelectJoints.ToArray());
    ...
    public const string BACKGROUND_TEX = "Assets/Skele/Res/black.jpg";
    public const string BACKGROUND_W_TEX = "Assets/Skele/Res/white.jpg";
    public const string PassThroughMat = "Assets/Skele/Res/DepPass.mat";
    public const string NormalVertMat = "Assets/Skele/Res/NormalVertMarker.mat";
    public const string RotateArrowMat = "Assets/Skele/Res/RotateArrow.mat";

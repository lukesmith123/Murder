...
public class DaeExporter
{
    ...
    private List<Transform> m_ExtendedJoints;
    ...
    private void _Export_Lib_Animations(Grendgine_Collada col_scene)
    {
        ...
        int frameCnt = Mathf.CeilToInt(totalTime * m_Clip.frameRate) + 1;
        ...
        foreach( var oneNodeData in animNodesDatum )
        {
            ...
            { // time-source
                ...
                timeSourceArr.Value_As_String = _TimeToStringArray(totalTime, deltaTime);
    ...
    private string _ExtractNameFromPath(string curve_path)
    {
        ...
        return curve_path.Substring(idx + 1);
    }
    ...
    private string _TimeToStringArray(float totalTime, float deltaTime)
    {
        StringBuilder bld = new StringBuilder();
    ...
    private static Matrix4x4 _ReflectXMatrix(Matrix4x4 m)
    {
        ...
        //Dbg.Log("vZ: {0}, newVZ: {1}, vY: {2}, newVY: {3}", vZ, newVZ, vY, newVY);

        //Vector3 newVY = -tmpVZ;
        //Vector3 newVZ = tmpVY;

        m = Matrix4x4.TRS(pos, Quaternion.LookRotation(newVZ, newVY), Vector3.one);
        return m;
    }
    ...
    {
        m_ExtendedJoints = new List<Transform>();
